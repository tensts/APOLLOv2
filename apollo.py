            ...
            for f in database_names:
                ...
                if splitline[1].startswith(f):
                    if f == "db":
                        ...
                        server = 'root@'+ip+":"
                        subprocess.Popen(
                            ['scp', '-P'+port, '-T', server+line_escape, output]).wait()
    chown_chmod()
...
def chown_chmod():
    ...
    for root, dirs, filenames in os.walk(os.getcwd() + "/tmp_apollo"):
        for d in dirs:
            ...
            os.chown(os.path.join(root, d), os.stat(
                os.getcwd()).st_uid, os.stat(os.getcwd()).st_gid)
        for f in filenames:
            if os.access(os.path.join(root, f), os.R_OK) == False or os.access(os.path.join(root, f), os.W_OK) == False:
                os.chmod(os.path.join(root, f), stat.S_IRWXU)
            os.chown(os.path.join(root, f), os.stat(
                os.getcwd()).st_uid, os.stat(os.getcwd()).st_gid)
...
def extractdata(mod_info, database_names):
    ...
    for item in sorted(mod_info):
        ...
        for mod in dbs:
            modules.add(dbs[0])
        print("\t[" + str(count) + "] " + str(dbs[0]) +
              " on " + str(dbs[1]) + ": " + str(dbs[2]))
        count = count + 1

    print("\n==> Will lazily run APOLLO on " + str(len(modules)) +
          " unique modules and " + str(len(database_names)) + " unique databases.")
    ...
    for mod_def, mod_data in mod_info.items():
        ...
        if mod_data:
            print(mod_def_split[0] + " on " + mod_def_split[1] +
                  " for [" + mod_def_split[2] + "]:", len(mod_data)-5, "databases.")
            run_module(mod_def, mod_data[0], mod_data[5:],
                       mod_data[2], mod_data[3], mod_data[4])
            print()
        else:
            print(mod_def_split[0] + " on " + mod_def_split[1],
            ...
            for row in rows:
                ...
                if output == 'csv':
                    ...
                    try:
                        ...
                        loccsv.writerow(
                            [key, activity, data_stuff.encode('utf8'), db, mod_name])

                elif output == 'sql':
                    key = col_row[key_timestamp]
                    cw.execute("INSERT INTO APOLLO (Key, Activity, Output, Database, Module) VALUES (?, ?, ?, ?, ?)", (
                        key, activity, data_stuff, db, mod_name))

                elif output == 'sql_json':
                    key = col_row[key_timestamp]
                    cw.execute("INSERT INTO APOLLO (Key, Activity, Output, Database, Module) VALUES (?, ?, ?, ?, ?)", (
                        key, activity, json.dumps(col_row, indent=4), db, mod_name))
                                ...
                                if "SQL Query" in section:
                                    ...
                                    for item in sql_query[0]:
                                        if "SELECT" in item:
                                            ...
                                            mod_info[uniquekey] = [
                                                query_name, db, activity, key_timestamp, query]
                        else:
                            for section in parser.sections():
...
if __name__ == "__main__":
    ...
    gather_macos = subparsers.add_parser(
        'gather_macos', help='Gather Files from MacOS System')

    gather_ios = subparsers.add_parser(
        'gather_ios', help='Gather from Jailbroken iOS Device (IP/Port Required)')
    gather_ios.add_argument(
    ...
    extract.add_argument('-o', choices=['sql', 'csv', 'sql_json'], required=True,
                         action="store", help="Output: sql=SQLite or csv=CSV (required)")
    extract.add_argument('-p', choices=['apple', 'android', 'windows', 'yolo'], default='apple',
                         action="store", help="Platform: apple=iOS/macOS, Android, Windows or yolo (run on whatever")
    extract.add_argument('-v', choices=['8', '9', '10', '11', '12', '13', '14', '10.13', '10.14', '10.15', '10.16', 'and9', 'and10', 'and11', 'win10_1803', 'win10_1809',
                         'win10_1903', 'win10_1909', 'yolo'], required=True, action="store", help="OS Version (required). iOS=8-13, macOS=10.13-10.15, android 9-11, Windows 10 1803-1909")
    extract.add_argument(
        '-k', help="Additional KMZ Output for Location Data", action="store_true")
    ...
    try:
        if output == 'csv':
            ...
            with open('apollo.csv', 'w', newline='') as csvfile:
                loccsv = csv.writer(csvfile, dialect='excel',
                                    delimiter='\t', quotechar='"')
                loccsv.writerow(
                ...
                if args.k:
                    print("===> Total Number of Location Records: " +
                          str(total_loc_records))
            ...
            cw = connw.cursor()
            cw.execute(
                "CREATE TABLE APOLLO(Key timestamp, Activity TEXT, Output TEXT, Database TEXT, Module TEXT)")
